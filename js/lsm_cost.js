...
function createExplanationPopup(Variables){
    ...
    if(document.getElementById('performance_conscious_checkbox').checked){
        ...
        text_div.innerHTML+="This key-value storage configuration is tailored to execute a workload comprising of "+v*100/sum+"% single-result lookups, "+r*100/sum+"% no-result lookups, and "+w*100/sum+"% writes on a base data of "+N+" entries each of size "+parseInt(document.getElementById("E").value.replace(/\D/g,''),10)+" bytes. To execute a sample workload of "+query+" queries, this configuration takes "+fixTime(Variables.latency)+" hour with an average throughput of "+Variables.throughput+" queries/sec. The cost you need to pay for this configuration is $"+Variables.cost+" per month which is slightly more than your budget of $"+parseInt(document.getElementById("cost").value.replace(/\D/g,''), 10)+" per month. We understand that you are a cost-conscious user!<br><br>";
        text_div.innerHTML+="This is "+a_or_an+" "+cloud_array[Variables.cloud_provider]+" configuration that will be deployed on "+Variables.VM_instance_num+" instance"+((Variables.VM_instance_num>1)?"s":"")+" of VMs of type "+Variables.VM_instance+". Within each VM, you can use "+Variables.Vcpu_num+" CPU cores and "+Variables.memory_footprint+" GB of memory for your workload. Regarding the in-memory allocation, "+(Variables.M_BF / 1024 / 1024 / 1024).toFixed(2)+" GB of memory will be reserved for storing bloom filters. For each level in sequence, we reserve"+bits_per_entry_text+fpr_text+(Variables.M_FP / 1024 / 1024 / 1024).toFixed(2)+" GB of memory will be reserved for storing fence pointers and "+(Variables.Buffer / 1024 / 1024 / 1024).toFixed(2)+" GB will be the size of the in-memory buffer. Within disk, the data structure will be organised into "+Variables.L+" level"+((Variables.L>1)?"s":"")+" that exponentially grow by a size ratio of "+Variables.T+". Within each level, there will be a maximum of "+Variables.K+" run"+((Variables.K>1)?"s":"")+" for the first "+(Variables.L-Variables.Y-1)+" level"+(((Variables.L-Variables.Y-1)>1)?"s":"")+" and "+Variables.Z+" run"+((Variables.Z>1)?"s":"")+" for the next "+(Variables.Y+1)+" level"+(((Variables.Y+1)>1)?"s":"")+". Disk-resident data will be compressed using "+Variables.compression_name+" compression scheme. For each lookup and write, the I/O cost is "+Variables.read_cost.toFixed(3)+" and "+Variables.update_cost.toFixed(3)+", respectively."

    }else{
        var text_div = document.createElement("div");
        text_div.innerHTML+="This key-value storage configuration is tailored to execute a workload comprising of "+v*100/sum+"% single-result lookups, "+r*100/sum+"% no-result lookups, and "+w*100/sum+"% writes on a base data of "+N+" entries each of size "+parseInt(document.getElementById("E").value.replace(/\D/g,''),10)+" bytes. To execute a sample workload of "+query+" queries, this configuration takes "+fixTime(Variables.latency)+" hour with an average throughput of "+Variables.throughput+" queries/sec. The cost you need to pay for this configuration is $"+Variables.cost+" per month which is within your budget of $"+parseInt(document.getElementById("cost").value.replace(/\D/g,''), 10)+" per month. We understand that you are a cost-conscious user!<br><br>";
        text_div.innerHTML+="This is "+a_or_an+" "+cloud_array[Variables.cloud_provider]+" configuration that will be deployed on "+Variables.VM_instance_num+" instance"+((Variables.VM_instance_num>1)?"s":"")+" of VMs of type "+Variables.VM_instance+". Within each VM, you can use "+Variables.Vcpu_num+" CPU cores and "+Variables.memory_footprint+" GB of memory for your workload. Regarding the in-memory allocation, "+(Variables.M_BF / 1024 / 1024 / 1024).toFixed(2)+" GB of memory will be reserved for storing bloom filters. For each level in sequence, we reserve"+bits_per_entry_text+fpr_text+(Variables.M_FP / 1024 / 1024 / 1024).toFixed(2)+" GB of memory will be reserved for storing fence pointers and "+(Variables.Buffer / 1024 / 1024 / 1024).toFixed(2)+" GB will be the size of the in-memory buffer. Within disk, the data structure will be organised into "+Variables.L+" level"+((Variables.L>1)?"s":"")+" that exponentially grow by a size ratio of "+Variables.T+". Within each level, there will be a maximum of "+Variables.K+" run"+((Variables.K>1)?"s":"")+" for the first "+(Variables.L-Variables.Y-1)+" level"+(((Variables.L-Variables.Y-1)>1)?"s":"")+" and "+Variables.Z+" run"+((Variables.Z>1)?"s":"")+" for the next "+(Variables.Y+1)+" level"+(((Variables.Y+1)>1)?"s":"")+". Disk-resident data will be compressed using "+Variables.compression_name+" compression scheme. For each lookup and write, the I/O cost is "+Variables.read_cost.toFixed(3)+" and "+Variables.update_cost.toFixed(3)+", respectively."
    }
